...
<?php	
			...
			foreach ($UserIds as $UserId) {
				...
				// NEW: (In supervisor only) hanya bisa memunculkan yang hanya anggota biasa
				if ($isAdmin == 0) {
				}
...
function insertUser() {
	global $Setting, $idUser, $nameUser, $isAdmin, $mode;
	
	// $idUser = (isset($_REQUEST['idUser']) ? trim($_REQUEST['idUser']) : '');
	// $nameUser = (isset($_REQUEST['nameUser']) ? trim($_REQUEST['nameUser']) : '');
	// $isAdmin = (isset($_REQUEST['isAdmin']) ? trim($_REQUEST['isAdmin']) : '');
...
function insertUserAction() {
	global $Setting, $idUser, $nameUser, $pwdUser, $isAdmin, $isSupervisor;
	
	// $idUser = (isset($_REQUEST['idUser']) ? trim($_REQUEST['idUser']) : '');
	// $nameUser = (isset($_REQUEST['nameUser']) ? trim($_REQUEST['nameUser']) : '');
	// $pwdUser = (isset($_REQUEST['pwdUser']) ? trim($_REQUEST['pwdUser']) : '');
	// $isAdmin = (isset($_REQUEST['isAdmin']) ? trim($_REQUEST['isAdmin']) : '');

	$bOK = $Setting->InsertUser($idUser, $nameUser, $pwdUser, $isAdmin, $isSupervisor);
...
function editUserAction() {
	...
	// $nameUser = (isset($_REQUEST['nameUser']) ? trim($_REQUEST['nameUser']) : '');
	// $pwdUser = (isset($_REQUEST['pwdUser']) ? trim($_REQUEST['pwdUser']) : '');
	// $isAdmin = (isset($_REQUEST['isAdmin']) ? trim($_REQUEST['isAdmin']) : '0');
	$n = strlen($pwdUser);
	$pwdUserDisp = "";
	for ($i = 0; $i < $n; $i++) {
		$pwdUserDisp .= "&bull;";
	...
	$bOK = $Setting->EditUser($idUser, $nameUser, $pwdUser, $isAdmin, $isSupervisor);
